...
public class SearchPage {
    ...
    private WebElement draftAllCheckbox;
    @FindBy(xpath = "//button[@class='btn btn-danger pagination-delete-btn']")
    private WebElement deleteButton;
    @FindBy(xpath = "//button[normalize-space()='Delete Job']")
    ...
    @FindBy(xpath = "//input[@id='new_job_title']")
    private WebElement postAJobTextBox;
    @FindBy(xpath = "//button[@id='exit-btn']")
    private WebElement exitButton;
    @FindBy(xpath = "(//button[text()='Invite to Job'])[1]")
    private WebElement inviteToJobButton01;
    @FindBy(xpath = "(//button[text()='Invite to Job'])[5]")
    private WebElement inviteToJobButton05;
    @FindBy(xpath = "(//button[contains(text(),'Invite to Existing Job')])[1]")
    private WebElement inviteToExistingJobLink;
    @FindBy(xpath = "(//a[contains(text(),'Create New Job')])[1]")
    private WebElement createNewJobLink;
    @FindBy(xpath = "//label[normalize-space()='Existing Job']")
    private WebElement existingJobText;
    @FindBy(xpath = "//button[@data-dismiss='modal'][normalize-space()='Cancel']")
    ...
    public void clearAllPostAJobs() {
        BaseClass.waitForVisibility(jobIcon, 20, SearchPage.driver);
        BaseClass.mouseOverOnElement(SearchPage.driver, jobIcon);
        BaseClass.waitForVisibility(draftLink, 20, SearchPage.driver);
        draftLink.click();
        BaseClass.staticWaitForVisibility(3000);
        BaseClass.javaScriptClick(SearchPage.driver, draftAllCheckbox);
        if (BaseClass.isElementPresent(SearchPage.driver, deleteButton)) {
            BaseClass.waitForVisibility(deleteButton, 20, SearchPage.driver);
            deleteButton.click();
            BaseClass.waitForVisibility(deleteJobButton, 20, SearchPage.driver);
            deleteJobButton.click();
        }

    }

    public void userCreatePostAJob() {
        BaseClass.waitForVisibility(postAJobButton, 20, SearchPage.driver);
        postAJobButton.click();
        BaseClass.waitForVisibility(postAJobTextBox, 20, SearchPage.driver);
        postAJobTextBox.sendKeys("Post a job");
        BaseClass.waitForVisibility(exitButton, 20, SearchPage.driver);
        exitButton.click();
        BaseClass.staticWaitForVisibility(5000);

    }

    public void validatePostAJob() {
        searchIcon.click();
        BaseClass.staticWaitForVisibility(3000);
        talentTab.click();
        BaseClass.staticWaitForVisibility(3000);
        List<WebElement> inviteJobListElement = SearchPage.driver.findElements(By.xpath("//button[text()='Invite to Job']"));
        for (WebElement element : inviteJobListElement) {
            Assert.assertEquals("Validate Invite Job Text", true, element.getText().contains("Invite to Job"));
        }

        inviteToJobButton01.click();
        BaseClass.staticWaitForVisibility(3000);
        inviteToExistingJobLink.click();
        BaseClass.staticWaitForVisibility(3000);
        Assert.assertEquals("Validate Existing Job text", "Existing Job", existingJobText.getText().trim());
        cancelButton.click();
        BaseClass.staticWaitForVisibility(2000);
        searchIcon.click();
        BaseClass.staticWaitForVisibility(3000);
        talentTab.click();
        BaseClass.staticWaitForVisibility(3000);
        BaseClass.javaScriptClick(SearchPage.driver, inviteToJobButton01);
        BaseClass.staticWaitForVisibility(2000);
        BaseClass.javaScriptClick(SearchPage.driver, createNewJobLink);
        BaseClass.waitForVisibility(postAJobText, 30, SearchPage.driver);
        Assert.assertEquals("Validate Post a Job text", "Post a Job", postAJobText.getText().trim());
